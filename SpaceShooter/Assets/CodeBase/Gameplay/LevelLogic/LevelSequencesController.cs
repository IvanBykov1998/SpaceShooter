...
namespace SpaceShooter
{
    public class LevelSequencesController : SingletonBase<LevelSequencesController>
    {
        ...
        public LevelProperties GetCurrentLoadedLevel()
        {
            ...
            Debug.LogError("Level prorerties not found!");
            return null;
        }

        public LevelProperties GetNextLevelProperties(LevelProperties levelProoerties)
        {
            for (int i = 0; i < LevelSequences.LevelsProperties.Length; i++)
            {
                if (LevelSequences.LevelsProperties[i].SceneName == levelProoerties.SceneName)
                {
            ...
            Debug.LogError("Level prorerties is last!");
            return null;
        }
    }
}